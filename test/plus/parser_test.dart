...
void main() {
  test('M3U_Plus single line - parsing all attribues', () async {
    final result = await parseFile(await FileUtils.loadFile(fileName: 'plus/single_line'));
  ...
  test('M3U_Plus multi line file', () async {
    final result = await parseFile(await FileUtils.loadFile(fileName: 'plus/multi_line'));
  ...
  test('M3U_Plus multi line file more headers', () async {
    final result = await parseFile(await FileUtils.loadFile(fileName: 'plus/multi_line_more_headers'));
    expect(result.epgUrl, 'http://www.teleguide.info/download/new3/jtv.zip');
    expect(result.entries.length, 5);
    expect(result.entries[0].title, "Первый канал");
  });
  ...
  test('M3U_Plus single line file with extgrp', () async {
    ...
    expect(result.epgUrl, 'http://epg.streaming-elbrus.su/xmltv/epg.xml.gz');
